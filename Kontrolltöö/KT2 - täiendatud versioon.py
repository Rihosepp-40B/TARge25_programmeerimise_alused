...
def not_found_add(filename: str, find: str) -> None:
    ...
    choice_ = input(f"Kas soovid '{find}' lisada kontaktide hulka? (y/'muu valik') ").lower()
    if choice_ == "y" or choice_ == "yes":
        choice_ = input(f"Vali: Kas '{find}' 1- Nimi või 2 Telefoninumber? Sisestamise katkestamiseks tee muu valik: ")
        if choice_ == "1":
            expand_phonebook(filename, find, "", 1)
        elif choice_ == "2":
            expand_phonebook(filename, "", find, 1)
        else:
            main_menu()
...
def ask_name_logic(contact_dict: dict) -> str:
    ...
    if check_value(contact_dict, name)[0] is True:
        ...
        if input(f"Kas soovid '{name}' andmed välja vahetada? (y/'Muu valik') ").lower() != "y":
            if input("Kas soovid sisestada uut isikut? (y/'Muu valik')").lower() != "y":
                return no_name
...
def expand_phonebook(filename: str, name="", phone_nr="", repeat=0) -> None:
...
def print_phonebook(filename: str) -> None:
    ...
    input("Jätkamiseks peamenüüsse vajuta ENTER: ")
    main_menu()
